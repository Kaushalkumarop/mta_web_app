...
function checkOverflowText() {
    ...
    var element1 = document.getElementsByClassName("overmarquee");
    for(let i = 0; i < element1.length; i++){
        // element = element[i];
        console.log(element1[i]);
        if (!element1[i]) break;
        if (isElementOverflowing(element1[i])) wrapContentsInMarquee(element1[i]);
    }
}
// checkOverflowText()
...
function update_user_details() {
    ...
    axios.post("./api/user_details.php", bodyFormData).then(function(response) {
...
function update_l_o_dat() {
    ...
    var bodyFormData = new URLSearchParams();
    bodyFormData.append("UID", UID);
    axios.post("./api/last_online.php", bodyFormData).then(function(response) {
        console.log(response.data);
    }).catch(function(error) {
        console.log(error);
    });
...
function login_check() {
    if (!store.get("motap_user_data")) // moving towards play page 
...
//# sourceMappingURL=login.869648c5.js.map
